<template>
  <ModalLayout
    ...
    :title="event ? 'Modification d\'experience' : 'Ajouter une nouvelle expreience'"
  >
          ...
          <div class="flex flex-col gap-4">
            <div class="flex flex-col w-full">
              ...
              <OnlyForm
                ...
                @update:modelValue="inputDataExperience.poste = $event.target.value"
                :error-input="isError"
              />
            </div>
            ...
            <div class="grid lg:grid-cols-2 grid-cols-1 gap-4 w-full">
              <div class="flex flex-col w-full">
                <label class="text-sm font-light" for="id-title">Type d'expérience </label>
                <formSelect
                  :modelValue="inputDataExperience.type_experience"
                  @update:modelValue="inputDataExperience.type_experience = $event.target.value"
              ...
              <div class="flex flex-col">
                <label class="text-sm font-light" for="id-title">Entreprise</label>
                <OnlyForm
                  placeholder="eg: Startup Diversion"
                  name="company"
                  :modelValue="inputDataExperience.company"
                  @update:modelValue="inputDataExperience.company = $event.target.value"
                  :error-input="isError"
                />
              </div>
            </div>
            ...
            <div class="grid lg:grid-cols-2 grid-cols-1 gap-4 w-full">
              <div class="flex flex-col">
                ...
                <formDate
                  ...
                  @update:modelValue="inputDataExperience.date_start = $event.target.value"
                  :error-input="isError"
                />
              </div>
              ...
              <div class="flex flex-col">
                ...
                <formDate
                  ...
                  @update:modelValue="inputDataExperience.date_finish = $event.target.value"
                  :to-day="true"
                  :error-input="isError"
                  :disabled="true"
                />
            ...
            <div class="flex flex-col w-full">
              ...
              <OnlyForm
                ...
                @update:modelValue="inputDataExperience.work_place = $event.target.value"
                :error-input="isError"
              />
            </div>
            ...
            <div class="flex flex-col w-full">
              <label class="text-sm font-light" for="id-title">Competence aquise</label>
              <VueSelect
...
const props = defineProps(['globalData', 'event', 'data'])
...
const addNewExprerience = async () => {
  ...
  try {
    const { error, context } = await new ProfileValidate().Experience(inputDataExperience)
    if (error) {
      state.loading = false
    ...
    //
    const { data } = await axios.post(URL.PROFILE_EXPERIENCE_CREATED, inputDataExperience)
    if (data) {
      state.loading = false
      ...
      //Send Notification
      new NotifUtils().success('Ajouter', 'Experience ajouter avec succès', 5000)
    }
  } catch (error) {
    state.loading = false
    console.log(error)
...
const updateExprerience = async () => {
  ...
  try {
    ...
    //
    const { data } = await axios.post(URL.PROFILE_EXPERIENCE_UPDATE, inputDataExperience)
    if (data) {
      state.loading = false
      data.experience.date_start_y = moment(data.experience.date_start).format('MMMM Y')
      data.experience.date_finish_y = !data.experience.currently_working
      ...
      //Send Notification
      new NotifUtils().success('Modifier', 'Experience modifier avec succès', 5000)
    }
  } catch (error) {
    state.loading = false
    console.log(error)
...
const deleteExperience = async () => {
  try {
    ...
    if (data) {
      ...
      //Send Notification
      new NotifUtils().success('Supprimer', 'Experience suprimer avec succès', 5000)
    }
  } catch (error) {
    console.log(error)
  }
