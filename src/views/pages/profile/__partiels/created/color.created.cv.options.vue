<template>
  <div class="flex absolute left-48 z-10 top-3">
    <!-- Display Color choose -->
    <div
      class="border-[1px] px-2 py-1 w-14 rounded-full flex items-center justify-between"
      @click="open ? (open = false) : (open = true)"
    >
      <div class="inline-flex w-4 h-4 rounded-sm" :class="CooColor.bg"></div>
      <i class="ri-arrow-down-s-line flex"></i>
    </div>

    <!-- Select Color -->
    <div class="grid grid-cols-4 absolute top-8 z-10 bg-gray-50 rounded-md p-2 w-44" v-if="open">
      <div
        v-for="color in OptionColorDataCv"
        class="inline-flex w-full h-8"
        :class="color.bg"
        @click="ChooseColorCv(color)"
      ></div>
    </div>
  </div>
</template>
...
const props = defineProps(['OptionColorDataCv', 'ChooseColorCv', 'CooColor'])
</script>
