<template>
  <section class="flex flex-col lg:p-4 lg:w-11/12 w-full m-auto" v-if="toAccess == 7">
          ...
          <div class="grid grid-cols-2 gap-4 w-full">
            <div class="flex flex-col w-full">
              ...
              <OnlyInput
                ...
                @update:modelValue="refc.contacts[0].value = $event.target.value"
                name="phone"
              />
            </div>
            <div class="flex flex-col w-full">
              ...
              <OnlyInput
                ...
                @update:modelValue="refc.contacts[1].value = $event.target.value"
                name="email"
              />
            </div>
          </div>
        </div>
        <BtnRemove label="suprimer cette reference" :index="index" @click="removeData(index)" />
      </div>
...
const props = defineProps(['toAccess', 'CV_Data', 'removeData', 'AddNewData'])
