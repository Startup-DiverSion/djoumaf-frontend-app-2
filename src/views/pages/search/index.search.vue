  ...
  <SideLayout>
    <template #content>
      ...
      <main class="flex flex-col w-full">
        <main
          class="flex flex-col lg:flex-row my-[80px] lg:w-11/12 w-full m-auto gap-4"
        >
          <section class="w-[20%] hidden lg:flex items-start">
            <CardMedia
              label="Filtre sur cette page"
              sticky="sticky top-20 w-full"
              :more="false"
            >
              <div
                class="flex flex-col gap-1 justify-center font-love text-base"
              >
          ...
          >
            ...
            <div class="" v-show="filterActived === 'jobs'">
              <div class="w-full lg:w-5/12 h-[50vh] flex items-center justify-center m-auto text-center" v-if="state_job.jobs && state_job.jobs.length === 0">
                <LoadingMessage
              ...
              <div
                ...
                v-if="
                  state_job.jobs &&
                  state_job.jobs.length !== 0 &&
                  state_job.loading === false
                "
              >
                <CradJobSearch v-if="state_job.jobs" :jobs="state_job.jobs" :envConfig="envConfig" />

                 <BtnMoreCard @click="SeeMoreJob" v-if="
                      state_job.paginted.current <
                        state_job.totalCount 
                    " label="Voir Plus" :loading="state_job.loading_paginied" />
              </div>
            </div>

            <!-- Personnes -->
            <div v-show="filterActived === 'person'" class="">
              <div class="w-full lg:w-5/12 h-[50vh] flex items-center justify-center m-auto text-center" v-if="state_profile.profile && state_profile.profile.length === 0">
                <LoadingMessage
            ...
            <div class="w-full" v-show="filterActived === 'company'">
              ...
              />
              <div class="w-full lg:w-5/12 h-[50vh] flex items-center justify-center m-auto text-center" v-if="state_profile_compagny.profile && state_profile_compagny.profile.length ===0">
                <LoadingMessage
              ...
              <BtnMoreCard @click="MoreSeeProfileCompagny" v-if="
                    ...
                    " label="Voir Plus" :loading="state_profile_compagny.loading_paginied" />
            </div>
          </section>

          <section
            class="w-full lg:w-[25%] flex flex-col gap-4 lg:sticky lg:top-20"
          >
            <CardSuggProfie />
          </section>
        </main>
